...
export default class TestCoveragePie extends Component{
 constructor(props){
     super(props);
 ...
 renderActiveShape = (props) => {
  ...
  return (
    <g>
      <text x={cx} y={cy} dy={8} fontSize={20} textAnchor="middle" fill={fill}>{payload.name}</text>
      <Sector
        cx={cx}
        cy={cy}
      ...
      <text  x={cx} y={cy} dy={30} fontSize={16} textAnchor="middle"  fill="#999"> {`${(percent * 100).toFixed(2)}%`}</text>
